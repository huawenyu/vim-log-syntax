...
syn match   ipaddrport              /:\d+\s/
syn match   ipaddrport1             /:443/
syn match   ipaddrport2             /:80/
syn match   ipaddrport3             /:8080/
syn match   ipaddrport4             /:8010/
...
syn match   rdocInlineURL2           /\(\w\+\(:\w\+\)\?@\)\?\([A-Za-z][-_0-9A-Za-z]*\.\)\{1,}\(\w\{2,}\.\?\)\{1,}\(:[0-9]\{1,5}\)\?\S*/
...
syn keyword  syntaxElementKeyword1  RET RETURN CONNECT ACCEPT TCP HTTP HTTPS FTP DNS EXPIRES READ WRITE SOCK GET CLOSE CLOSED FREE HOST NULL NIL DENY
...
hi def link  ipaddr                 Number
hi def link  ipaddrport             Constant
hi def link  ipaddrport1            Constant
hi def link  ipaddrport2            Constant
hi def link  ipaddrport3            Constant
hi def link  ipaddrport4            Constant
...
hi def link  rdocInlineURL2         Identifier
